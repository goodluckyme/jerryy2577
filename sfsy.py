...
# Cron: 15 8,16 * * *
# new Env("顺丰速运")
# export sfsyUrl=https://mcs-mimp-web.sf-express.com/mcs-mimp/share/weChat/shareGiftReceiveRedirect?xxxx=xxxx&xxxxx;原本有跑顺丰速运的不用加配置
...
class SFExpress:
    ...
    async def get_task_list(self):
        ...
        data = await self.post('/mcs-mimp/commonPost/~memberNonactivity~integralTaskStrategyService~queryPointTaskAndSignFromES', params)
        return data
    ...
    async def fetch_award(self, task):
        ...
        data = await self.post('/mcs-mimp/commonPost/~memberNonactivity~integralTaskStrategyService~fetchIntegral', params)
        return data
    ...
    async def bee_receive_honey(self, task_type):
        ...
        data = await self.post('/mcs-mimp/commonPost/~memberNonactivity~receiveExchangeIndexService~receiveHoney', params)
        return data
